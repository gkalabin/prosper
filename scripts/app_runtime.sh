...
# by scripts/start.sh (production container entrypoint) and e2e/start.sh
...
# wait_for_socket polls PROSPER_GRPC_SOCKET_PATH once per second up to
...
wait_for_socket() {
  ...
  while [ "$i" -lt "$socket_wait_timeout_seconds" ]; do
    if [ -S "$PROSPER_GRPC_SOCKET_PATH" ]; then
      return 0
    fi
    if ! kill -0 "$pid" 2>/dev/null; then
      log "backend exited before opening $PROSPER_GRPC_SOCKET_PATH"
      return 1
    fi
    sleep 1
    i=$((i + 1))
  done
  log "ERROR: backend did not open $PROSPER_GRPC_SOCKET_PATH within ${socket_wait_timeout_seconds}s"
  return 1
}
...
app_run() {
  ...
  log "backend ready at $PROSPER_GRPC_SOCKET_PATH"
