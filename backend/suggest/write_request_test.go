...
func moneyCand(v int64, conf int32) *prosperv1.MoneyCandidate {
	return &prosperv1.MoneyCandidate{Confidence: conf, ValueNanos: v}
}

func strCand(v string, conf int32) *prosperv1.StringCandidate {
	return &prosperv1.StringCandidate{Confidence: conf, Value: v}
}

func tsCand(t time.Time) *prosperv1.TimestampCandidate {
	return &prosperv1.TimestampCandidate{Confidence: confidenceObserved, Value: timestamppb.New(t)}
}

func formCand(ft prosperv1.FormType) *prosperv1.FormTypeCandidate {
	return &prosperv1.FormTypeCandidate{Confidence: confidenceObserved, Value: ft}
}

func sharingCand(st prosperv1.SharingType) *prosperv1.SharingTypeCandidate {
...
func TestWriteRequestExpenseNotShared(t *testing.T) {
	d := &prosperv1.TransactionDraft{
		...
		FormType:      []*prosperv1.FormTypeCandidate{formCand(prosperv1.FormType_FORM_TYPE_EXPENSE)},
		Timestamp:     []*prosperv1.TimestampCandidate{tsCand(fixedTime)},
		Amount:        []*prosperv1.MoneyCandidate{moneyCand(24_990_000_000, confidenceObserved)},
		AccountFromId: []*prosperv1.IdCandidate{idCand(7)},
		CategoryId:    []*prosperv1.IdCandidate{{Confidence: confidenceLearned, Value: 3}},
		Vendor:        []*prosperv1.StringCandidate{strCand("AMAZON", confidenceWeak), strCand("Amazon", confidenceLearned)},
	}
...
func TestWriteRequestExpenseSharedFillsCompanionAndOwnShare(t *testing.T) {
	d := &prosperv1.TransactionDraft{
		...
		CategoryId:     []*prosperv1.IdCandidate{{Confidence: confidenceLearned, Value: 3}},
		Vendor:         []*prosperv1.StringCandidate{strCand("AMAZON", confidenceWeak), strCand("Amazon", confidenceLearned)},
		SharingType:    []*prosperv1.SharingTypeCandidate{sharingCand(prosperv1.SharingType_SHARING_TYPE_PAID_SELF_SHARED)},
		Companion:      []*prosperv1.StringCandidate{strCand("Jane", confidenceLearned)},
		OwnShareAmount: []*prosperv1.MoneyCandidate{moneyCand(12_495_000_000, confidenceLearned)},
	}
	req, err := WriteRequestFromDraft(d)
	if err != nil {
		t.Fatalf("WriteRequestFromDraft: %v", err)
...
func TestWriteRequestIncome(t *testing.T) {
	d := &prosperv1.TransactionDraft{
		...
		FormType:    []*prosperv1.FormTypeCandidate{formCand(prosperv1.FormType_FORM_TYPE_INCOME)},
		Timestamp:   []*prosperv1.TimestampCandidate{tsCand(fixedTime)},
		Amount:      []*prosperv1.MoneyCandidate{moneyCand(1_000_000_000, confidenceObserved)},
		AccountToId: []*prosperv1.IdCandidate{idCand(9)},
		CategoryId:  []*prosperv1.IdCandidate{{Confidence: confidenceLearned, Value: 5}},
		Payer:       []*prosperv1.StringCandidate{strCand("ACME", confidenceWeak)},
	}
	req, err := WriteRequestFromDraft(d)
	if err != nil {
		t.Fatalf("WriteRequestFromDraft: %v", err)
