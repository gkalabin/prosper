...
func TestTopCategoriesRanksByFrequency(t *testing.T) {
	...
	got := h.topCategories(prosperv1.FormType_FORM_TYPE_EXPENSE, "", 5)
...
func TestTopCategoriesRelaxesNameThenType(t *testing.T) {
	f := newFixture()
	...
	got := h.topCategories(prosperv1.FormType_FORM_TYPE_EXPENSE, "Starbucks", 2)
	// The vendor-specific 7 ranks ahead of the globally more frequent 9
	// pulled in only after the vendor ranking runs out.
	if want := []int32{7, 9}; !slices.Equal(got, want) {
		t.Errorf("categories = %v, want %v with the vendor match ranked first", got, want)
	}
}

func TestTopCategoriesPrefersRecent(t *testing.T) {
	f := newFixture()
	// The vendor's old habit (9, twice) predates the recent window; the
	// single recent recording under 7 outranks it.
	f.addExpense(10, day(-200), "Cafe", 9, ledgerAccountChecking, 100)
	f.addExpense(11, day(-200), "Cafe", 9, ledgerAccountChecking, 100)
	f.addExpense(12, day(-5), "Cafe", 7, ledgerAccountChecking, 100)
	h := newHistory(f.snapshot(), dayZero)

	got := h.topCategories(prosperv1.FormType_FORM_TYPE_EXPENSE, "Cafe", 5)
	if want := []int32{7, 9}; !slices.Equal(got, want) {
		t.Errorf("categories = %v, want %v with the recent category first", got, want)
	}
}

func TestTopCategoriesMatchesNameIgnoringCaseAndSpaces(t *testing.T) {
	f := newFixture()
	f.addExpense(10, day(-5), " Starbucks ", 7, ledgerAccountChecking, 100)
	f.addExpense(11, day(-5), "Rent", 9, ledgerAccountChecking, 100)
	h := newHistory(f.snapshot(), dayZero)

	got := h.topCategories(prosperv1.FormType_FORM_TYPE_EXPENSE, "sTaRbUcKs", 1)
	if want := []int32{7}; !slices.Equal(got, want) {
		t.Errorf("categories = %v, want %v matched despite case and spacing differences", got, want)
	}
}
...
func TestTopCategoriesStopsOnceWantSatisfied(t *testing.T) {
	...
	got := h.topCategories(prosperv1.FormType_FORM_TYPE_EXPENSE, "Starbucks", 2)
	// Both wanted categories come from the vendor ranking, so it is
...
func TestTopCategoriesTruncatesToWant(t *testing.T) {
	...
	got := h.topCategories(prosperv1.FormType_FORM_TYPE_EXPENSE, "", 1)
	// A single ranking holds both categories; the result is capped at want.
	if want := []int32{7}; !slices.Equal(got, want) {
...
func TestTopCategoriesReturnsAllWhenWantExceedsAvailable(t *testing.T) {
	...
	got := h.topCategories(prosperv1.FormType_FORM_TYPE_EXPENSE, "", 10)
	if want := []int32{7, 9}; !slices.Equal(got, want) {
		t.Errorf("categories = %v, want %v with every distinct category", got, want)
	}
}

func TestTopCategoriesKeepsFormType(t *testing.T) {
	f := newFixture()
	f.addExpense(10, day(-5), "Cafe", 7, ledgerAccountChecking, 100)
	f.addIncome(11, day(-5), "ACME", 3, ledgerAccountChecking, 100)
	h := newHistory(f.snapshot(), dayZero)

	got := h.topCategories(prosperv1.FormType_FORM_TYPE_INCOME, "Nobody", 5)
	// Relaxation stops at the form type, so the expense category never
	// leaks into an income proposal.
	if want := []int32{3}; !slices.Equal(got, want) {
		t.Errorf("categories = %v, want %v with the income ranking retained", got, want)
	}
}
