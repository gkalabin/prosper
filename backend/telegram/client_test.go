...
func TestClientFailsFastOnLongRetryAfter(t *testing.T) {
	var calls atomic.Int32
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		calls.Add(1)
		w.Write([]byte(`{"ok":false,"error_code":429,"description":"Too Many Requests: retry after 3600","parameters":{"retry_after":3600}}`))
	}))
	defer server.Close()

	start := time.Now()
	if _, err := testClient(server).SendMessage(context.Background(), 1, "hi", nil); err == nil {
		t.Fatal("expected a flood wait longer than maxRetryBackoff to surface as an error")
	}
	if elapsed := time.Since(start); elapsed > maxRetryBackoff {
		t.Errorf("call blocked for %s, want no wait on a flood limit", elapsed)
	}
	if calls.Load() != 1 {
		t.Errorf("expected no retry of a flood wait, got %d calls", calls.Load())
	}
}
