  ...
  return (
    <div className="col-span-3">
      <label
        ...
        className="block text-sm font-medium text-gray-700"
      >
        {share == SharingType.PAID_SELF_SHARED && <>My share</>}
        {share == SharingType.PAID_OTHER_OWED && (
          <>My share (which I owe {getValues('expense.payer') || 'them'})</>
        )}
        {share == SharingType.PAID_OTHER_REPAID && (
    ...
    <>
      {share == SharingType.PAID_OTHER_OWED && (
        <div className="text-xs">
          ...
          <ButtonLink
            onClick={() => {
              setValue('expense.shareType', SharingType.PAID_OTHER_REPAID);
              setValue(
                'expense.repaidFromAccountId',
                getValues('expense.accountId')
              );
              setValue(
                'expense.repaidTimestamp',
      ...
      {share == SharingType.PAID_OTHER_REPAID && (
        <div className="text-xs">
          ...
          <ButtonLink
            onClick={() => {
              setValue('expense.shareType', SharingType.PAID_OTHER_OWED);
              setValue(
                'expense.accountId',
                getValues('expense.repaidFromAccountId')
              );
            }}
          >
            I owe them money
        ...
        <div className="flex items-center">
          <div className="flex">
            <Switch
              checked={shared}
              onChange={() => {
                if (share == SharingType.PAID_SELF_SHARED) {
                  setValue(
                    'expense.shareType',
                    SharingType.PAID_SELF_NOT_SHARED
                  );
                ...
                if (share == SharingType.PAID_SELF_NOT_SHARED) {
                  setValue('expense.shareType', SharingType.PAID_SELF_SHARED);
